...
TEST(getFieldTest, getExistingFields) {
    ...
    EXPECT_EQ(Application::getField(0, line), "f0");
    EXPECT_EQ(Application::getField(1, line), "f1");
    EXPECT_EQ(Application::getField(2, line), "f2");
}

TEST(getFieldTest, getNonExistingFields) {
    EXPECT_EQ(Application::getField(2, "0\t1"), "");
    EXPECT_EQ(Application::getField(1, "hello"), "");
    EXPECT_EQ(Application::getField(5, "h\te\tl\tl\to"), "");
}
...
TEST(fillMapGenderByIDTest, basicInput) {
    ...
    std::stringstream ss;
    ss << "0\tMale\n1\tFemale\n2\tOther";
    Application::fillMapGenderByID(ss, &genderByID);
...
TEST(printArtistTest, emptyInput) {
    ...
    Application::printArtist(input, genderByID, ssOut);
    std::string out;
    ssOut >> out;
    EXPECT_EQ(out, "");
}
